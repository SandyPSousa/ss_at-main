...
export default function Register() {
  ...
  const handleRegister = (e) => {
    e.preventDefault();
    const user = createUser(form?.email, form?.password);
    if (!user) {
      e.target.reset();
      Swal.fire({
        title: "Error!",
        text: t("Failed to create account"),
        icon: "error",
    ...
    >
      <Card sx={{ width: "100%", maxWidth: "360px" }}>
        ...
        <CardActions>
          ...
          >
            <Button variant="contained" type="submit" form="register">
              {t("Create account")}
            </Button>
            <Button variant="text" onClick={(e) => navigate("/login")}>
              {t("Log in")}
            </Button>
          </Box>
        </CardActions>
